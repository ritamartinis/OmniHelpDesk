...
// Query para buscar todos os tickets abertos pelo ADMIN enquanto id_utilizador_autor
$sqlBase = "SELECT t.*, u.nome AS nome_tecnico
                                   FROM Tickets t
...
$tickets_admin = $consulta->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <div class="container mt-5">
        ...
        <?php if (count($tickets_admin) > 0): ?>
            <div class="d-flex flex-wrap justify-content-center">
                <?php foreach ($tickets_admin as $ticket): ?>
                    ...
                    <div class="card m-2 <?php echo ($ticket['estado'] === 'Fechado') ? 'ticket-fechado' : ''; ?> <?php echo ($ticket['estado'] === 'Cancelado') ? 'ticket-cancelado' : ''; ?>"
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                ...
                                <?php if ($ticket['estado'] === 'Fechado'): ?>
                                    <button type="button" class="btn btn-warning btn-sm m-1"
                                        onclick="showReabrirTicketModal(<?php echo $ticket['id_ticket']; ?>)">Reabrir</button>
                                <?php endif; ?>
                            ...
                            <p class="card-text">
                                ...
                                </strong><?php echo htmlspecialchars($ticket['nome_tecnico'] ?? 'Por atribuir'); ?><br>
                                <strong>Fechado a:
                                </strong><?php echo $ticket['data_encerramento'] ? date('d-m-Y H:i', strtotime($ticket['data_encerramento'])) : 'Não Encerrado'; ?><br>
                            </p>
                            <div class="separator"></div>
                            <div class="d-flex justify-content-center">
                                <a href="responder_ticket.php?id_ticket=<?php echo $ticket['id_ticket']; ?>"
                                    class="btn btn-secondary btn-sm m-1">Responder</a>
                                <a href="descarregar_ticket.php?id_ticket=<?php echo $ticket['id_ticket']; ?>"
                                    class="btn btn-success btn-sm m-1" target="_blank">Descarregar Ticket</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <?php else: ?>
            ...
            <div class="text-center error-box">
                <p>De momento, não possui nenhum ticket pessoal.</p>
            </div>
        <?php endif; ?>
    </div>
        
    <!-- Modal para Reabrir o Ticket -->
    <div class="modal fade" id="confirmModalReabrirTicket" tabindex="-1"
        aria-labelledby="confirmModalReabrirTicketLabel" aria-hidden="true">
